#!/bin/bash
# Collect clean / ours / latent / beat traces at K=16 and plot latent potential figures.
set -euo pipefail
cd "$(dirname "$0")/../.."
...
LOG="${LOGDIR_ROOT}/${TASK}_potential_K16.log"
exec > >(tee -a "${LOG}") 2>&1
...
run_collect() {
  ...
  local ckpt="$2"
  echo ""
  echo "========== [viz collect K=16] ${TASK} / ${name} =========="
  MUJOCO_GL=egl MUJOCO_EGL_DEVICE_ID=${MUJOCO_EGL_DEVICE_ID} \
  "${PYTHON}" eval_backdoor.py \
    "${COMMON[@]}" \
    "ckpt_path=${ckpt}" \
    "viz.model_name=${name}" \
    "logdir=logdir/metaworld/backdoor/_viz_eval_${TASK}_K16_${name}"
}

echo "[$(date '+%F %T')] latent potential K=16 pipeline started for ${TASK}"
...
echo "[$(date '+%F %T')] latent potential K=16 pipeline complete for ${TASK}"
echo "  traces: ${OUT}"
echo "  figures: ${FIG}"
